    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
      <div className="bg-white border border-slate-200 rounded-3xl max-w-2xl w-full max-h-[85vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="p-6 border-b border-slate-100 flex items-start justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-xs font-semibold text-emerald-600 uppercase tracking-wider">
              ...
              <span>Verified Source Messages ({resolvedMessages.length})</span>
            </div>
          ...
          {resolvedMessages.length === 0 ? (
            ...
            resolvedMessages.map((msg, index) => (
              ...
              >
                ...
                <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-100 text-sm text-slate-900 whitespace-pre-wrap leading-relaxed">
                  {msg.content}
                </div>
              </div>
            ))
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between text-xs text-slate-500">
          <span>Traceability index: matches original export timestamps</span>
          <button
            onClick={onClose}
            className="px-4 py-2 font-medium text-white bg-slate-900 hover:bg-slate-800 rounded-xl transition-colors"
          >
            Close
          </button>
        </div>
