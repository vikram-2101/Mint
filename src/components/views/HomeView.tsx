    ...
    <div className="max-w-4xl mx-auto px-6 py-8 space-y-6 animate-in fade-in duration-300">
      ...
      {/* <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="flex items-center gap-3 p-3.5 rounded-2xl bg-white border border-slate-200 shadow-2xs">
          ...
          <div>
            <div className="text-xs font-bold text-slate-900">Save time</div>
      ...
      {/* Error Message */}
      {errorMessage && (
        <div className="flex items-start gap-3 p-4 rounded-2xl bg-red-50 text-red-700 border border-red-200 text-xs">
          <AlertCircle className="h-4 w-4 shrink-0 mt-0.5" />
          <span>{errorMessage}</span>
        </div>
      )}

      {/* 3. Upload Zone Card matching Screenshot 1 */}
      <div
        onDragOver={handleDragOver}
        onDragLeave={handleDragLeave}
        onDrop={handleDrop}
        className={`border-2 border-dashed rounded-3xl p-10 sm:p-14 text-center transition-all bg-white shadow-sm space-y-6 ${
          isDragging
      ...
      >
        ...
        <div className="h-12 w-12 rounded-2xl bg-emerald-100 text-emerald-600 flex items-center justify-center mx-auto relative shadow-inner">
          ...
          <div className="absolute -bottom-1 -right-1 h-6 w-6 rounded-full bg-emerald-600 text-white flex items-center justify-center ring-2 ring-white shadow-sm">
        ...
        <div>
          <button
            ...
            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white text-sm font-semibold shadow-md shadow-emerald-700/20 hover:scale-[1.02] active:scale-[0.98] transition-all"
          >
        ...
        <p className="text-[11px] text-slate-400 pt-2">
          Supported format: WhatsApp .txt export (without media) &bull; Up to 25
          MB
        </p>
      </div>

      {/* 4. Or Try A Sample Chat */}
      <div className="space-y-4 text-center pt-2">
        ...
        <button
          ...
          className="inline-flex items-center justify-between gap-4 max-w-sm w-full mx-auto px-5 py-3 rounded-2xl bg-white border border-slate-200 hover:border-emerald-300 hover:bg-emerald-50/40 text-slate-800 text-xs font-semibold shadow-2xs hover:shadow-xs transition-all"
        >
